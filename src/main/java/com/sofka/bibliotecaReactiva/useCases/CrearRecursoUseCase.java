...
public class CrearRecursoUseCase implements SaveRecurso {
    ...
    @Autowired
    public CrearRecursoUseCase(MapperUtils mapperUtils, RecursoRepository repositorio) {
    ...
    public Mono<String> apply(RecursoDto recursoDto) {
        return repositorio.save(mapperUtils.mapperToRecurso(null).apply(recursoDto))
                .map(Recurso::getId);
    }
}
